...
function sendSms($phoneNumber="15000000000", $smsCode="111111", $operation) {
    
    //此处需要替换成自己的AK信息
    $accessKeyId = C("yourAccessKeyId");
    $accessKeySecret = C("yourAccessKeySecret");
    //短信API产品名
    $product = "Dysmsapi";
    //短信API产品域名
    ...
    if ($operation === 'register') {
        // 签名
        $smsSign="阿里桑门禁";
        $templateCode = "SMS_77270022";
        $templateParam = "{\"number\":\"$smsCode\"}";
    } else if ($operation === 'findPassword') {
        $smsSign="阿里桑门禁";
        $templateCode = "SMS_77280023";
        $templateParam = "{\"number\":\"$smsCode\"}";
    } else {
