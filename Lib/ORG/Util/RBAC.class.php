...
class RBAC {
	...
	static public function checkAuth($authId,$module,$action){
		if(null===$authId)  $authId  =  $_SESSION[C('ADMIN_AUTH_KEY')] ? $_SESSION[C('ADMIN_AUTH_KEY')] : $_SESSION[C('USER_AUTH_KEY')];
		$appName = APP_NAME;
        ...
        if($_SESSION[C('ADMIN_AUTH_KEY')] != $authId) {
            if(C('USER_AUTH_TYPE')==2) {
                //加强验证和即时验证模式 更加安全 后台权限修改可以即时生效
                //通过数据库进行访问检查
                $accessList = RBAC::getAccessList($_SESSION[C('USER_AUTH_KEY')]);
